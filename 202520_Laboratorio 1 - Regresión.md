...
Define grouping mechanism to grpup by player not game

\# of goals - added -> maybe goals per game

one hot encoding for players with more than one position

pos1 pos2 pos3

1-0-1

or number of time a position was playeed (figure out how to handle it)

for the specialized model, maybe we can have duplicate data if we sepcialize it by position...

(assume htye are comparable in time, all played 90 mins)

if there are correlations between variables explicativas, we remove one of them. that is less correlated to the objective variable...

Revisar Supuestos en diapsitivas////

debe havber relacion lineal entre variables explicativas y variable target
